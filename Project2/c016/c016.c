...
void htInsert(tHTable *ptrht, tKey key, tData data) {
    ...
    if (tmp != NULL)
        ...
        tmp = (*ptrht)[index];
        (*ptrht)[index] = new_item; //place it to the start of the list
...
void htDelete(tHTable *ptrht, tKey key) {
    ...
    if (item != NULL) {
        if (previous == NULL)
            (*ptrht)[index] = item->ptrnext;    //first item
        else
            previous->ptrnext = item->ptrnext;  //further item
        free(item);
    }
}
...
** který tyto položky zabíraly, a uvede tabulk
 * u do počátečního stavu.
*/
