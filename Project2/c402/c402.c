...
**/
    if ((*RootPtr) == NULL) {   //empty tree
        ...
        tBTNodePtr actual = (*RootPtr);
        tBTNodePtr father = NULL;
        while (actual != NULL) {
            if (Content < actual->Cont) {       //bigger go left
                father = actual;
                actual = actual->LPtr;
            } else if (Content > actual->Cont) {  //smaller go right
                father = actual;
                actual = actual->RPtr;
            } else                                //equal
                return;
        ...
        if (new_node != NULL) {
            ...
            new_node->LPtr = NULL;
            new_node->RPtr = NULL;
            if (father->Cont > Content) //left
                father->LPtr = new_node;
            else                        //right
                father->RPtr = new_node;
        }
    }
}
...
**/
    if (RootPtr != NULL) {
        ...
        while (!SEmptyP(&stackP)) {
            ...
            if (STopPopB(&stackB)) {    //decide to go right or left
                SPushB(&stackB, FALSE);
...
**/
    if (RootPtr != NULL) {
        ...
        while (!SEmptyP(&stack)) {
            tmp = STopPopP(&stack);
            if (tmp == NULL)             //already done
                continue;
            SPushP(&stack, tmp->LPtr);   //push both nodes
