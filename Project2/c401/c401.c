...
**/
    ...
    if (K == RootPtr->Key){
        *Content = RootPtr->BSTNodeCont;    //found
        return TRUE;
    }

    if (K > RootPtr->Key)
        return BSTSearch(RootPtr->RPtr,K,Content);  //go right
    else
        return BSTSearch(RootPtr->LPtr,K,Content);  //go left
}
...
**/
    if (*RootPtr == NULL) {
        tBSTNodePtr new_node = malloc(sizeof(struct tBSTNode));
        if (new_node != NULL) {
...
**/
    ...
    if ((*RootPtr)->RPtr != NULL) //go more right
...
**/
    if ((*RootPtr) != NULL) {
        ...
        tmp = NULL;
        if ((*RootPtr)->Key < K)
            BSTDelete(&(*RootPtr)->RPtr, K);   // bigger go right
        else if((*RootPtr)->Key > K)
            BSTDelete(&(*RootPtr)->LPtr, K); //smaller go left
        else {  //found
            tmp = (*RootPtr);
